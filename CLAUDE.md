...
Static HTML/CSS site deployed to Neocities. Task (`Taskfile.yml`) wraps the `neocities` Ruby gem. Formatting via Prettier from the `Brewfile` (`brew bundle`).
...
task format     # prettier --write --ignore-unknown . (prettier from the Brewfile)
